(function(){
  ...
  function initialState(){
    ...
    const audit=[{id:'a-1',at:'2026-09-20T08:20:00Z',actorId:'u-admin',action:'DEMO_READY',entityType:'SYSTEM',entityId:'V7.0',details:{mode:'GitHub Pages full test + AI Form Studio + pluggable digital signature capability/adapters'}}];
  ...
  async function demoApi(rawUrl,opts={}){
    ...
    if(m&&method==='POST')throw new Error('V7.0: workflow analysis phải đi qua Real AI Router.');
    ...
    if(path==='/api/ai/providers/models'&&method==='POST')throw new Error('V7.0: model discovery phải đi qua browser real-provider adapter.');
    if(path==='/api/ai/providers/probe'&&method==='POST')throw new Error('V7.0: capability probe phải đi qua browser real-provider adapter.');
    if(path==='/api/ai/providers/test'&&method==='POST')throw new Error('V7.0: provider test mô phỏng đã bị vô hiệu hóa; phải kiểm tra provider thật.');
    if(path==='/api/ai/assist'&&method==='POST')throw new Error('V7.0: AI assist mô phỏng đã bị vô hiệu hóa; cần provider AI thật.');
